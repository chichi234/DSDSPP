...
//            mBeaconName.setFilters(new InputFilter.LengthFilter[]{new InputFilter.LengthFilter(25)});
            ...
            setViewEnable(SPPConnect.getInstance().getConnectState() == ConnectState.READY_RW);
        }
        return mRootView;
    }
    ...
    public void receive(byte[] data, int len) {
        DebugLog.i("receive:" + new String(data, 0, len));
        if (new String(data, 0, len).contains("OK")) {
            if (getActivity() != null) {
...
//                mListener.onExitSetState();
    ...
    private void sendData(String command) {
        ...
        SPPConnect.getInstance().write(command);
    }
