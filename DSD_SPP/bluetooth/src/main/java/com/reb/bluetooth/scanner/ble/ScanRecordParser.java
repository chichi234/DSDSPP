...
//		Log.d(TAG, serviceUUID);
    ...
    public static ExtendedBluetoothDevice decodeDeviceAdvData(byte[] data, BluetoothDevice device, int rssi, UUID filterUUID) {
        ExtendedBluetoothDevice exDevice = null;
        boolean isValid = filterUUID == null;
        String type = "";//
            ...
            for (int index = 0; index < packetLength; index++) {
                ...
                switch (fieldName) {
                    ...
                    case SERVICES_COMPLETE_LIST_128_BIT:
                        // 过滤服务
                        isValid |= decodeFilterService(fieldName, fieldLength, data, index, filterUUID);
                        break;
                    case FLAGS_BIT:
                        // 设备类型
    ...
    private static boolean decodeFilterService(int fieldName, int fieldLength, byte[] data, int index, UUID filterUUID) {
        boolean valid = false;
        if (filterUUID != null) {
            String uuid = filterUUID.toString();
            if (fieldName == SERVICES_MORE_AVAILABLE_16_BIT || fieldName == SERVICES_COMPLETE_LIST_16_BIT) {
                for (int i = index + 1; i < index + fieldLength - 1; i += 2)
                    valid = valid || decodeService16BitUUID(uuid, data, i, 2);
            } else if (fieldName == SERVICES_MORE_AVAILABLE_32_BIT || fieldName == SERVICES_COMPLETE_LIST_32_BIT) {
